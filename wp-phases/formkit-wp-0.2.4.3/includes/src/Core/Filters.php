...
final class Filters {
    ...
    private function date(mixed $v, mixed $arg): string {
        $fmt = $arg ?: 'Y-m-d';
        if ($v instanceof \DateTimeInterface) return $v->format($fmt);
        $ts = is_numeric($v) ? (int)$v : strtotime((string)$v);
        if ($ts === false) return '';
        return date($fmt, $ts);
    }
    private function upper(mixed $v): string { return mb_strtoupper((string)$v); }
    private function lower(mixed $v): string { return mb_strtolower((string)$v); }
}
